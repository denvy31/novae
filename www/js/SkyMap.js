...
function SkyMap() {
  ...
  const draw = useCallback(() => {
    ...
    g.addColorStop(0, "#05070f"); g.addColorStop(0.7, "#0a1226"); g.addColorStop(1, "#10203e");
    ...
    if (showPlanets) {
      ...
      f.bodies.forEach((b) => {
        ...
        if (pr > 16) { if (pl.rings) P.drawRings(ctx, p[0], p[1], pr, light, false); P.drawPlanetTextured(ctx, p[0], p[1], pr, pl, 0.2, light); if (pl.rings) P.drawRings(ctx, p[0], p[1], pr, light, true); }
    ...
    // reticle (drawn unrotated, screen-fixed)
    if (motion) {
      ctx.strokeStyle = "rgba(255,255,255,0.55)"; ctx.lineWidth = 1.3;
      ctx.beginPath(); ctx.arc(w / 2, h / 2, 16, 0, 7);
      ctx.moveTo(w / 2 - 24, h / 2); ctx.lineTo(w / 2 - 6, h / 2); ctx.moveTo(w / 2 + 6, h / 2); ctx.lineTo(w / 2 + 24, h / 2);
      ctx.moveTo(w / 2, h / 2 - 24); ctx.lineTo(w / 2, h / 2 - 6); ctx.moveTo(w / 2, h / 2 + 6); ctx.lineTo(w / 2, h / 2 + 24); ctx.stroke();
      const all = [].concat(planetHit, namedHit.map((n) => ({ x: n.x, y: n.y, name: n.s.name })), satHit.map((s) => ({ x: s.x, y: s.y, name: s.data.name })), dsHit.map((d) => ({ x: d.x, y: d.y, name: d.data.name || ("M" + d.data.m) })));
      ...
      reticleRef.current = best ? (best.data ? best.data.name : best.name) : (cbc && f.consts[cbc.idx] ? f.consts[cbc.idx].name : "");
  ...
  useEffect(() => {
    ...
    waitAE().then((ok) => {
      ...
      if (navigator.geolocation) navigator.geolocation.getCurrentPosition((pos) => { if (!mounted) return; obs.current = { lat: pos.coords.latitude, lon: pos.coords.longitude, elev: pos.coords.altitude || 30, label: "Ma position" }; rerender(); refresh(); }, () => {}, { timeout: 8000 });
      loadSats();
    });
    const tick = setInterval(() => { if (!mounted) return; const c = clock.current; if (c.live) c.date = new Date(); else if (c.rate) c.date = new Date(c.date.getTime() + c.rate * 1000); setClockLabel(fmtClock(c.date, c.live)); if (motionOn.current) setReticleName(reticleRef.current); refresh(); }, 1000);
  ...
  const useMyLocation = () => { if (!navigator.geolocation) { setStatus("Géolocalisation indisponible."); return; } navigator.geolocation.getCurrentPosition((pos) => { obs.current = { lat: pos.coords.latitude, lon: pos.coords.longitude, elev: pos.coords.altitude || 30, label: "Ma position" }; rerender(); refresh(); }, () => setStatus("Position refusée."), { enableHighAccuracy: true, timeout: 8000 }); };
  ...
  const handleOrientation = useCallback((e) => {
    ...
    let az = Math.atan2(lx, ly) / DEG; az = ((az % 360) + 360) % 360;
  ...
  const motionLoop = useCallback(() => {
    ...
    drawRef.current();
    const cp = centerPlanetRef.current, nm = cp ? cp.data.name : null;
    if (hudRef.current !== nm) { hudRef.current = nm; setHud(cp); }
  ...
  return React.createElement("div", { className: "skymap-wrap" },
    ...
    React.createElement("div", { className: "sky-status-bar" },
      ...
      React.createElement("div", { className: "clock-controls" },
    ...
    React.createElement("div", { className: "sky-jumps" },
      React.createElement("span", { className: "jump-label" }, "Regarder :"),
    ...
    motion && React.createElement("div", { className: "motion-status" },
      React.createElement("div", null, reticleName ? "🎯 " + reticleName : "🧭 Pointez le téléphone vers le ciel"),
      React.createElement("div", { className: "motion-hint" }, "Décalé ? Glissez pour aligner · ",
        React.createElement("button", { className: "recalib-btn", onClick: recalibrate }, "Recalibrer"))),
