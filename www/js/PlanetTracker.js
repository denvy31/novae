  ...
  useEffect(() => {
    ...
    function drawMap() {
      ...
      NV.planets.forEach((p) => {
        ...
        const pr = Math.max(2.5, Math.min(11, Math.pow(p.diam, 0.32) / 7));
        const m = Math.hypot(cx - px, cy - py) || 1;
        P.drawBall(ctx, px, py, pr, p.color, { x: (cx - px) / m, y: (cy - py) / m });
        if (p.rings) { ctx.strokeStyle = "rgba(220,205,160,0.85)"; ctx.lineWidth = 1.4; ctx.beginPath(); ctx.ellipse(px, py, pr * 2, pr * 0.7, 0.5, 0, Math.PI * 2); ctx.stroke(); }
    ...
    function drawCloseup() {
      ...
      const drawMoon = (mo) => {
        ...
        P.drawBall(ctx, mo.x, mo.y, mr, mo.m.color, light);
